...
- On certain Linux distributions, Mathematica’s bundled GMP [appears to be incompatible](https://github.com/peraro/finiteflow/issues/6) with Flint, causing a `LibraryFunction::load` error.  This can be resolved by either using [flint-finiteflow-dep](https://github.com/peraro/flint-finiteflow-dep) instead of the official Flint library, or by disabling Flint entirely via `-DFFLOW_USE_FLINT=0` during `cmake` configuration.
